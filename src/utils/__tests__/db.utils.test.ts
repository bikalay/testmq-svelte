...
describe("DB Utils", () => {
  ...
  describe("count", () => {
    it("should return 0 for empty collection", async () => {
      const _count = await count(STORE_TEMPERATURE_NAME);
      expect(_count).toBe(0);
    });
  });
  describe("insertMany", () => {
    it("should insert data successful", async () => {
      await insertMany(STORE_TEMPERATURE_NAME, [
        {v: 1, t: 2},
        {v: 3, t: 4},
      ]);
      const _count = await count(STORE_TEMPERATURE_NAME);
  ...
  describe("clear", () => {
    it("should remove all data from an collection", async () => {
      await insertMany(STORE_TEMPERATURE_NAME, [{v: 5, t: 6}]);
      const _count = await count(STORE_TEMPERATURE_NAME);
      expect(_count).toBe(1);
      await clear(STORE_TEMPERATURE_NAME);
      const _count2 = await count(STORE_TEMPERATURE_NAME);
  ...
  describe("query", () => {
    it("should return all records", async () => {
      ...
      const result = await query<ItemData>(STORE_TEMPERATURE_NAME);
    ...
    it("should return records by query", async () => {
      ...
      const result2 = await query<ItemData>(STORE_TEMPERATURE_NAME, {
        indexName: "t",
  ...
  describe("getMinimalValue", () => {
    it("should return minimal value", async () => {
      ...
      const minValue = await getMinimalValueByField(STORE_TEMPERATURE_NAME,  "t");
  ...
  describe("getMaximalValue", () => {
    it("should return max value", async () => {
      ...
      const minValue = await getMaximalValueByField(STORE_TEMPERATURE_NAME, "t");
